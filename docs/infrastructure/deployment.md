...
- **Shared cPanel hosting** — there is **no Docker**. Every container becomes a native
  ...
  `scheduler` → a cron, Mailpit → real SMTP, Caddy → Apache.
...
phpMyAdmin behind a `debug` profile. Only Caddy is exposed — on `:80` + `:443`, terminating TLS
itself (see below).
...
| `caddy` | Apache + the shipped `public/.htaccess` |
