...
trait ProvidesOptions
{
    ...
    /**
     * Wildcard-escaped name match — mirrors App\Filters\Primitives\AbstractFilter
     * so the picker search behaves like the list-page search filters (MySQL LIKE
     * is case-insensitive by default; SQLite is used in tests). Only the ESCAPE
     * clause is inlined and must spell the backslash escape char per driver.
     *
    ...
    protected function applyNameSearch(Builder $query, string $term): void
    {
        ...
        $query->whereRaw(
            $grammar->wrap('name').' like ? escape '.$escape,
            ['%'.escape_like($term).'%'],
        );
    }
}
