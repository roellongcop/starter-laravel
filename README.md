...
- **Settings** — typed spatie/laravel-settings groups (System/Email/Image/Notification)
  with typed forms; **Themes** — light/dark token palettes applied live.
- **Files** — uploads via medialibrary on a private disk, gated download/preview.
...
- **Backups** (spatie/laravel-backup), **My Exports** (csv/xls/xlsx/pdf), **My Imports**
  (upload→preview→process) — all via queued jobs with status tracking + gated downloads.
...
`s3` (SeaweedFS) via their `*_DISK_DRIVER` env var. Downloads always go through gated
